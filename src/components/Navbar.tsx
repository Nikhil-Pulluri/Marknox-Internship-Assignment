...
function Navbar() {
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      if (scrollTop > 70) {
        // Scrolling down
        if (navbar) navbar.style.top = '-70px' // Adjust the value based on your navbar height
      } else {
        // Scrolling up
  ...
  return (
    <div className="bg-transparent pt-5 z-[1] mt-[30px] transition-[top] duration-500 transform -translate-y-full">
      <nav className="fixed w-full z-20 top-0 start-0" id="navbar">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-center mx-auto p-4" id="nav-content">
          <div className="items-center justify-center hidden w-full md:flex md:w-auto sm:order-1" id="navbar-sticky">
            <ul className="dark flex flex-col gap-2 p-4 md:p-0 mt-4 font-medium md:space-x-40 rtl:space-x-reverse md:flex-row md:mt-0 md:border-0">
              <li>
                <Link
                  href="/"
